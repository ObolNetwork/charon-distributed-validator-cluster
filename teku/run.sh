...
# Teku validator client entrypoint for the Platåberget devnet.
# Renders Teku's proposer config from the charon-generated canonical config when
# available: entries only carry fields diverging from default_config, absent
# fields fall back to it. The network spec is derived from the beacon node
# (charon) via --network=auto, since this is a custom devnet.

TEKU=$(command -v teku || echo /opt/teku/bin/teku)
