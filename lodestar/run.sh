...
# Lodestar validator client entrypoint for the Platåberget devnet.
# Lays the charon keystores out in lodestar's persisted-keys layout, renders
# per-validator proposer settings from the charon-generated proposer-config.json,
# then runs the validator client against the local charon node. The network is
# a custom devnet, so --paramsFile is used instead of a named --network.
